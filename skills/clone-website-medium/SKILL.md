...
description: Reverse-engineer and clone one or more websites with the same pixel-perfect rigor as clone-website-full — exhaustive CSS/behavior extraction, per-component spec files, full interaction sweeps — but built entirely inline by one agent, sequentially, with NO subagents, NO agent teams, and NO git worktrees, and with explicit token/tool-call efficiency habits (batched browser calls, no redundant re-extraction, no reprinting large JSON into chat). This is the middle of three clone-website tiers (full / medium / lite) — same thoroughness as full, lower usage overhead, solo execution. Use whenever the user wants a thorough/pixel-perfect website clone but explicitly says not to spawn subagents or worktrees, wants everything done "by you directly", wants it efficient but not shallow, or is working somewhere subagents/worktrees aren't available or wanted. If the user is fine with a multi-agent parallel build, use clone-website-full instead; if they want speed/cost over exhaustive fidelity, use clone-website-lite. Provide one or more target URLs as arguments.
