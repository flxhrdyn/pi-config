---
name: clone-website-full
description: Reverse-engineer and clone one or more websites in one shot — extracts assets, CSS, and content section-by-section and proactively dispatches parallel builder agents in worktrees as it goes. This is the heaviest of three clone-website tiers (full / medium / lite) — use it when the user wants maximum pixel-perfect fidelity and is fine with a multi-agent build (parallel builder subagents, git worktrees, a merge step). Triggers on "clone/replicate/rebuild/copy this site", "pixel-perfect clone", or explicit requests to use agent teams/subagents for the build. If the user says not to use subagents or worktrees, use clone-website-medium instead; if they want it fast and cheap over exhaustive, use clone-website-lite. Provide one or more target URLs as arguments.
